...
def run_ml(): # similar to perform_ML3(), compute featImp and confusion matrix foldbyfold added
    ...
    clf = XGBClassifier(n_estimators=600, max_depth=70, learning_rate=0.01, subsample=0.8, colsample_bytree=0.8,
                              nthread=4, scale_pos_weight=estimate)
    ...
    res.to_csv('xgb_metrics.csv')

    # write predictions to file
    feat_imp_df.to_csv('xgb_feature_importance.csv', index=False)
    fold_pred.to_csv('xgb_prediction.csv', index=False)
    print('Completed!')
